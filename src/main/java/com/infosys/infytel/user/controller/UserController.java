	...
	// Create a new buyer
		...
		public ResponseEntity<String> createBuyer(@RequestBody BuyerDTO buyerDTO) throws Exception{
			...
			try {
				...
				String successMessage=environment.getProperty("BUYER_SUCCESS")+" With ID "+buyerId;
		...
		// finding buyer
				...
				public BuyerDTO findBuyer(@PathVariable Integer buyerId) {
					try {
						logger.info("find request for buyer {} with buyerId{}",buyerId);
						BuyerDTO buyerDTO=userService.findBuyerId(buyerId);
						return buyerDTO;
					}
					catch(Exception e) {
						throw new ResponseStatusException(HttpStatus.BAD_REQUEST,environment.getProperty(e.getMessage()),e);
